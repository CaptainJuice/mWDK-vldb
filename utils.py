...
def pplot_single(embedding,true_labels,name):
    ...
    ax.scatter(embedding[:, 0], embedding[:, 1], c=true_labels, s=3, cmap=newcmp,)
    plt.axis('equal')
    ax.tick_params(labelsize=8)
    # ax.set_title(name)
...
def pplot_single_seperate(embedding, true_labels, name):
    # embedding = np.around(embedding, 3)
    colors = ['C0', 'C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8', 'C9']
    pos = 171
    ...
    for i in range(7):
        t = [p for p in range(embedding.shape[0]) if true_labels[p]==i]
        myemb = embedding[t]
        mylab = true_labels[t]
        ax = fig.add_subplot(pos)
        pos+=1
        ax.scatter(myemb[:, 0], myemb[:, 1], s=3, color=colors[i], )
        plt.axis('equal')
        ax.tick_params(labelsize=8)
        ax.set_title('clasee {}'.format(i+1))
